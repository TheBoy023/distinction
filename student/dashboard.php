...
try {
    ...
    // Retrieve student's information from the database using the session user ID
    $stmt_user = $pdo->prepare("SELECT student_name, student_id, department FROM users WHERE email = :email");
    $stmt_user->bindParam(':email', $_SESSION['email'], PDO::PARAM_STR);
    $stmt_user->execute();
    ...
    $department = $user_info['department'] ?? 'Unknown';
    ...
    <div class="container">
        <aside class="sidebar">
            <ul>
                ...
                <li><a href="input_grades.php?<?$_SESSION['email']?>">Input Grades for Deans's List</a></li>
