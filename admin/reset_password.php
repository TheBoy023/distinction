...
if (isset($_GET['token']) && !empty($_GET['token'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT admin_id, email, reset_token_expiry FROM admins WHERE reset_token = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['reset_token'])) {
    ...
    if ($new_password !== $confirm_password) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT id, email FROM admins WHERE reset_token = ?");
            $stmt->bind_param("s", $reset_token);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($result->num_rows === 1) {
                ...
                $update_stmt = $conn->prepare("UPDATE admins SET password = ?, reset_token = NULL, reset_token_expiry = NULL WHERE id = ?");
    ...
    <div class="container">
        ...
        <div class="left-section">
            <div class="quote">
                ...
                <br><span>- Deanslist Security</span>
            </div>
        </div>
